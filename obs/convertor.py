...
class Convertor(object):
        ...
        for grant in grants:
            ...
            if grant.get('grantee') is not None:
                ...
                if grantee.get('group') is not None:
                    ...
                    if group_val:
                        granteeEle = ET.SubElement(grantEle, 'Grantee', None if self.is_obs else attrib)
    ...
    def trans_restore(self, days, tier):
        ...
        if tier is not None:
            glacierJobEle = ET.SubElement(root, 'GlacierJobParameters')
    ...
    def trans_copy_part(self, **kwargs):
        headers = {}
        self._put_key_value(headers, self.ha.copy_source_header(), kwargs.get('copySource'))
    ...
    def parseListObjects(self, xml, headers=None):
        ...
        if contents is not None:
            for node in contents:
                ...
                owner_id = self._find_item(node, './Owner/ID')
                owner_name = None if self.is_obs else self._find_item(node, './Owner/DisplayName')
                owner = Owner(owner_id=owner_id, owner_name=owner_name)
                isAppendable = self._find_item(node, 'Type')
                key_entry = Content(key=key, lastModified=DateTime.UTCToLocal(lastmodified), etag=etag, size=util.to_long(size), owner=owner, storageClass=storage, 
    ...
    def parseGetBucketLocation(self, xml, headers=None):
        root = ET.fromstring(xml)
        location = self._find_item(root, './Location' if self.is_obs else './LocationConstraint')
