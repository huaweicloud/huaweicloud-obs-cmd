...
class SchemaTransformer(object):
    ...
    * Properties: `type`, `description`, `required`, `enum`
    ...
    could call it with the shorthand JSON like so::
        ...
        $ obscmd foo bar --baz arg1=Value1,arg2=5 arg1=Value2

    """
    JSON_SCHEMA_TO_HCMD_TYPES = {
        'object': 'structure',
        'array': 'list',
    }
    ...
    def transform(self, schema):
        """Convert JSON schema to the format used internally by the OBS CLI.
        ...
        :return: The transformed model in a form that can be consumed
            internally by the OBS CLI.  The dictionary returned will
    ...
    def _transform_structure(self, schema, shapes):
        ...
        for key, value in schema['properties'].items():
            current_type_name = self._json_schema_to_hcmd_type(value)
    ...
    def _transform_map(self, schema, shapes):
        ...
        for attribute in ['key', 'value']:
            type_name = self._json_schema_to_hcmd_type(schema[attribute])
    ...
    def _transform_list(self, schema, shapes):
        ...
        member_type = self._json_schema_to_hcmd_type(schema['items'])
    ...
    def _populate_initial_shape(self, schema):
        shape = {'type': self._json_schema_to_hcmd_type(schema)}
    ...
    def _json_schema_to_hcmd_type(self, schema):
        if 'type' not in schema:
            raise ParameterRequiredError("Missing required key: 'type'")
        type_name = schema['type']
        return self.JSON_SCHEMA_TO_HCMD_TYPES.get(type_name, type_name)
